...
def search(glamfe,gphife,cx,cy,il,ir,jl,jr):
    # Conducts a binary search to find which grid box contains CHS point cx,cy
...
def getboxij(glamfe,gphife,lon,lat,nmax=-1,searchmore=False,cache=None):
    if cache is not None and os.path.exists(cache):
        ...
        # The goal here is to find the model grid box that each CHS point falls inside
...
def binstobathy(boxi,boxj,x,y,z,NX,NY):
    # Now we use the boxi,boxj information to assemble the binned bathy on NEMO grid
        ...
        if np.any(idx):   # j coordinate matches
            ...
            for i in range(NX): # i coordinate matches
                ...
                if np.any(idx):
                    ...
                    # - the USGS depths are ztmp[idx]
                    bmin[j,i]    = np.min(ztmp[idx])
                    bmax[j,i]    = np.max(ztmp[idx])
                    bmean[j,i]   = np.mean(ztmp[idx])
                    bmedian[j,i] = np.median(ztmp[idx])
                    bcount[j,i]  = len(ztmp[idx])
